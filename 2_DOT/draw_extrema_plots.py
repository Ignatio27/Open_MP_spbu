...
def read_table(path: str) -> pd.DataFrame:
    ...
    # в этом bm_table.csv разделитель — запятая
...
def plot_per_size(df: pd.DataFrame) -> None:
    ...
    metrics = [
        ("time_sec", "Время, с"),
        ("speedup_vs_seq", "Ускорение (seq/parallel)"),
    ]

    for size, df_size in df.groupby("vec_size"):
        for col, y_label in metrics:
            plt.figure()
            for name, df_method in df_size.groupby("variant"):
                ordered = df_method.sort_values("threads")
                plt.plot(
                    ordered["threads"],
                    ordered[col],
                    marker="o",
...
def plot_aggregated(df: pd.DataFrame) -> None:
    ...
    metrics = [
        ...
        ("speedup_vs_seq", "Среднее ускорение (по всем n)"),
    ]

    for col, y_label in metrics:
        plt.figure()
        for name, df_method in mean_stats.groupby("variant"):
            ordered = df_method.sort_values("threads")
            plt.plot(
                ordered["threads"],
                ordered[col],
...
def main(argv: list[str]) -> None:
    if len(argv) < 2:
        print("Использование: python plot_bm.py <benchmarks.csv>")
        raise SystemExit(1)
